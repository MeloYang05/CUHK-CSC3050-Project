...
def tobinary(number,length,sighed=1): #sighed=0 represents unsighed, =1 represents sighed
...
# get the name of the test file and output file
...
for line in lines:
    if line[0] in R_functions.keys(): #judge whether R type
        ...
        if line[0] == "lui":
            ...
            im=tobinary(line[2],16,sighed) #transform the address to binary format
            rs="00000"
        elif line[0] in rt_rs_im_list:
            rt=MIPS_Register[line[1]]
            rs=MIPS_Register[line[2]]
            im=tobinary(line[3],16,sighed)
        elif line[0] in rt_imrs_list:
            rt=MIPS_Register[line[1]]
            p1=line[2].index("(")
            p2=line[2].index(")")
            try:
                im=tobinary(line[2][:p1],16,sighed) 
            except:
                im=tobinary(0,16,sighed)
            rs=MIPS_Register[line[2][p1+1:p2]]
        elif line[0] in rs_label_list:
            rs=MIPS_Register[line[1]]
            im=tobinary(label_dict[line[2]],16,sighed)
            if line[0] == "bgez":
                rt="00001"
            else:
                rt="00000"
            ...
            rs=MIPS_Register[line[1]]
            rt=MIPS_Register[line[2]]
            im=tobinary(label_dict[line[3]],16,sighed)
